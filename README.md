...
1.  **`model()`**: Create a dynamic model based on input parameters.
    ...
    -   **`levels`**: An integer specifying the number of levels of
        threats intensities in the model.
...
1.  **`solve()`**: Solve a mathematical optimization problem using
    ...
    -   **`a`**: A list containing information about the optimization
        ...
        parameters.
    -   **`solver`**: A character specifying the solver to be used
        (“gurobi” or “cbc”).
...
actions interact over space and time. The function has several key
parameters:

-   **`input`**: the structured object created by the `instance()`
    function, containing spatial data, threat configuration, species
    presence, and cost information.
-   **`levels`**: the number of discrete threat intensity levels
    **including 0**, which represents the absence of threat. For
    example, `levels = 4` means that threats can range from level 0 (no
    threat) up to level 3 (maximum intensity).
-   **`periods`**: the number of time periods in the planning horizon
    (e.g., 10 periods).
-   **`budget`**: the amount of budget available per time period for
...
Key parameters of the `solve()` function include:

-   **`a`**: the model object returned by `model()`.
-   **`solver`**: character string indicating which solver to use.
    Options are:
...
-   **`time_limit`**: maximum time allowed for the solver to run, in
    seconds.
-   **`gap_limit`**: the relative optimality gap at which to stop
    solving (e.g., 0.01 = 1%).
...
<img src="sol_animation_b100_actions.gif" width="100%" style="display: block; margin: auto;" />
...
<img src="sol_animation_b300_actions.gif" width="100%" style="display: block; margin: auto;" />
...
<img src="sol_animation_b500_actions.gif" width="100%" style="display: block; margin: auto;" />
...
<img src="sol_animation_b600_actions.gif" width="100%" style="display: block; margin: auto;" />
